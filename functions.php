 ...
 *
 * */
	require_once 'inc/WalkerNavMenuCustom.php';
	require_once 'inc/WalkerNavMenuCustomMobile.php';
...
function register_post_types(){
    register_post_type('doctor', array(
        ...
        'labels' => array(
            ...
            'not_found'          => 'Специалист не найдены', // если в результате поиска ничего не было найдено
            'not_found_in_trash' => 'Специалист не найден', // если не было найдено в корзине
        ...
        'hierarchical'        => false,
        'supports'            => [ 'title', 'editor', 'comments' ], // 'title','editor','author','thumbnail','excerpt','trackbacks','custom-fields','comments','revisions','page-attributes','post-formats'
        'taxonomies'          => ['category'],
        'has_archive'         => true,
        'rewrite'             => true,
        'query_var'           => true,
...
//Трансфрмация в код для ресурса Youtube из ссылки на видео
function youtubeLinkTransform($link_video_raw){

    //$link_video_raw = 'https://www.youtube.com/embed/uAoMCfePwOM';
    //https://www.youtube.com/watch?v=6MNzYK4I0wM - пример копирования из браузера

    preg_match('/\?v=([^\/$]+)/i',$link_video_raw,$rezult);
    $link_video = ($link_video_raw and $rezult[1]) ? 'https://www.youtube.com/embed/'.$rezult[1] : false;

    return $link_video;
}

//function doctors_type_query( $query ) {
//    if ( is_post_type_archive('doctors') && is_main_query() )
//        $query->set( 'posts_per_page', 15);
//}
//add_action( 'pre_get_posts', 'doctors_type_query' );
...
//Включим коммментарии по умолчанию
...
require_once 'inc/CustomCommentWalker.php';
